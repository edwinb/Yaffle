...
doUpdates : {auto c : Ref Ctxt Defs} ->
            ...
            Updates -> List SourcePart ->
            Core (List SourcePart)
doUpdates ups [] = pure []   -- no more tokens to update, so we are done
-- if we have a name that acts as an as-pattern then do not update it
doUpdates ups (Name n :: AsPattern :: xs)
    = pure $ Name n :: AsPattern :: !(doUpdates ups xs)
...
doUpdates ups (AsPattern :: LBrace :: xs)
    = pure $ AsPattern :: LBrace :: !(doUpdates ups xs)
-- if we have an LBrace (i.e. `{`), handle its contents
doUpdates ups (LBrace :: xs)
    ...
    = let (ws, nws) = span isWhitespace xs in
        case nws of
               ...
               case nws' of
                  ...
                  RBrace :: rest' =>
                    pure (LBrace :: ws ++
                          ...
                          !(doUpdates ups (Name n :: ws' ++ RBrace :: rest'))
                          )
          ...
          _ => pure (LBrace :: [] ++ !(doUpdates ups xs))
