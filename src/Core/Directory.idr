...
firstAvailable : {auto c : Ref Ctxt Defs} ->
                 List String -> CoreE err (Maybe String)
...
findDataFile fname
    = do d <- getDirs
         ...
         Just f <- firstAvailable fs
            | Nothing => throw (TTFileErr ("Can't find data file " ++ fname ++
                                               " in any of " ++ show fs))
         pure f
...
readDataFile fname
    = do f <- findDataFile fname
         Right d <- coreLift $ readFile f
            | Left err => throw (SystemFileErr f err)
         pure d
    ...
    = do d <- getDirs
         let fs = map (\p => p </> fname)
                      (lib_dirs d ++ map (\x => x </> "lib")
                                         (extra_dirs d))
         Just f <- firstAvailable fs
            | Nothing => throw (TTFileErr ("Can't find library " ++ fname))
         pure f
...
export
covering
ensureDirectoryExists : String -> CoreFile ()
ensureDirectoryExists dir
    = do Right _ <- coreLift $ mkdirAll dir
            | Left err => throw (SystemFileErr dir err)
         pure ()
...
export
getExecFileName : {auto c : Ref Ctxt Defs} -> String -> CoreE err String
